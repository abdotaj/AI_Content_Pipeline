...
ابحث تحديداً عن أي بقايا عامية (مصرية، شامية، خليجية، سعودية) وصحّحها إلى الفصحى، بما في ذلك:
- أي فعل مضارع يبدأ بحرف الباء الزائدة (بيحكم، بتحكم، بيعرف، بنعمل، بتصحى، بتلاقي) ← صحّحه إلى الفصيح بلا باء (يحكم، تحكم، يعرف، نعمل، تستيقظ، تجد)
- هذا ينطبق أيضاً على الجمل الافتتاحية بصيغة المخاطب ("تخيّل أنك...") — تبقى فصحى بالكامل، لا عامية أبداً
- كلمات مثل: مش، إيه، عشان، ليش، شلون، وايد، حد (بمعنى أحد)، شي (بمعنى شيء)، تصحى، تلاقي ← استبدلها بمقابلها الفصيح
...
- ممنوع تماماً أي فعل مضارع يبدأ بحرف الباء الزائدة (مثل: بيحكم، بتحكم، بيعرف، بنعمل، بتصحى، بتلاقي) — هذه صيغة عامية في كل اللهجات وليست فصحى أبداً، حتى في الجمل الافتتاحية بصيغة المخاطب ("تخيّل أنك..."). الفعل المضارع الفصيح لا يبدأ بباء زائدة مطلقاً: يحكم، تحكم، يعرف، نعمل، تستيقظ، تجد — بلا باء. ممنوع أيضاً: تصحى (الصواب: تستيقظ)، تلاقي (الصواب: تجد)
...
def _write_arabic_from_research(en_script: dict, ar_research: dict, target_minutes: float | None = None) -> str:
    ...
    entity_lock = (
        ...
        f"[قواعد الحقائق — انتهاكها فشل فوري]\n"
        f"1. استخدم الحقائق الواردة في الكتلة البحثية فقط. "
        f"لا تخترع كتباً أو أفلاماً أو جوائز أو وثائقيات أو شخصيات لم تُذكر في البحث.\n"
        f"2. إذا نفدت الحقائق قبل اكتمال الكلمات المطلوبة، "
        f"أنهِ الفصل باستنتاج قوي ومقنع — لا تملأ المساحة بمعلومات مخترعة.\n\n"
    )
    ...
    if _anim_mode:
        ...
        _tts_reminder = (
            ...
            "[قاعدة نحوية إلزامية]: ممنوع تماماً أي فعل مضارع يبدأ بحرف الباء الزائدة "
            "(مثل: بيحكم، بتحكم، بيعرف، بنعمل، بيروح، بتقول) — هذه صيغة عامية في كل اللهجات وليست فصحى أبداً. "
            "الفعل المضارع الفصيح لا يبدأ بباء زائدة مطلقاً: يحكم، تحكم، يعرف، نعمل، يذهب، تقول — بلا باء. "
            "ممنوع أيضاً: تصحى (الصواب: تستيقظ)، تلاقي (الصواب: تجد)، إنك/إنه بمعنى أنّك/أنّه.\n"
        )
    else:
        _tts_reminder = (
