...
def generate_voiceover_edgetts(script_text: str, filename: str, language: str = "english") -> str:
    ...
    if language.lower() == "arabic":
        voice = "ar-SA-HamedNeural"   # HamdanNeural doesn't exist — was causing "No audio received"
        rate  = "+10%"                # +25% was too fast for Arabic comprehension
    else:
...
def generate_voiceover(script_text: str, filename: str, language: str = "english") -> str:
    ...
    # Priority 1: OpenAI TTS (primary — highest quality, Arabic: marin/gpt-4o-mini-tts, English: alloy/tts-1)
    ...
    # Priority 2: edge-tts (fallback — free, no quota, lower quality)
    print("[Voice] Trying edge-tts (fallback)...")
    result = generate_voiceover_edgetts(script_text, filename, language)
    if result:
        return result
    print("[TTS] edge-tts fallback also failed")
...
def run_full_pipeline(
    ...
    # ── Quality processing — disabled ──────────────────────────────────────
    # video_quality.process_video() re-encoded the already-correct 1080x1920
    # output through two no-op passes: the "long" variant targets 1920x1080
    # landscape (pad filter crashes on every call — vertical input is already
    # taller than that target) and the "short" variant re-encoded the FULL
    # long video (not a trimmed clip) at 1080x1920 — a scale/crop no-op since
    # the source is already that size, costing ~30 min per language for zero
    # visual change, and it overwrote the real ~90s short_clip_path (set above
    # via cut_short_clip) with this mislabeled full-length file. Both variants
    # are redundant now that renders are produced directly in the target
    # 1080x1920 format, so this whole pass is skipped.
